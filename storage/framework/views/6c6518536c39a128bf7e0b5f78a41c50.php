        ...
        <div style="flex: 1; overflow-y: auto; min-height: 0; background: white;" class="custom-scrollbar">
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $conversations; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $conversation): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                <button wire:click="selectConversation('<?php echo e($conversation->customer_phone); ?>')"
                    style="width: 100%; padding: 12px; text-align: left; border: none; border-bottom: 1px solid #f3f4f6; cursor: pointer; <?php echo e($selectedPhone === $conversation->customer_phone ? 'background-color: #f0fdf4;' : 'background: white;'); ?>">
                    <div style="font-size: 14px; font-weight: 500; color: black;">
                        <?php echo e($conversation->customer_phone); ?>
            ...
            <div id="template-modal" style="display: none; position: absolute; inset: 0; background: rgba(0,0,0,0.5); z-index: 50; align-items: center; justify-content: center; padding: 2rem;">
                <div style="background: white; border-radius: 12px; width: 100%; max-width: 500px; display: flex; flex-direction: column; overflow: hidden; box-shadow: 0 20px 25px -5px rgba(0,0,0,0.1);">
                    ...
                    <div style="padding: 1rem; overflow-y: auto; max-height: 400px;">
                        <p style="font-size: 0.875rem; color: #4b5563; margin-bottom: 1rem;">Selecciona una plantilla para enviar. Los campos <?php echo e(1); ?>, <?php echo e(2); ?>, etc. deben ser completados.</p>
                        
                        <div style="display: flex; flex-direction: column; gap: 10px;">
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = \App\Models\WhatsAppTemplate::where('store_id', Auth::user()->store_id)->get(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $tpl): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                                <div style="border: 1px solid #e5e7eb; border-radius: 8px; padding: 10px; cursor: pointer; hover:bg-gray-50;" 
                                     onclick="selectTemplate('<?php echo e($tpl->id); ?>', '<?php echo e($tpl->name); ?>', <?php echo e(json_encode($tpl->parameters_map)); ?>)">
                                    <div style="font-weight: bold; font-size: 13px; color: #2563eb;"><?php echo e($tpl->name); ?></div>
                                    <div style="font-size: 12px; color: #6b7280;"><?php echo e($tpl->body_preview); ?></div>
                                </div>
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::endLoop(); ?><?php endif; ?><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::closeLoop(); ?><?php endif; ?>
                        </div>
...
<script>
    ...
    function selectTemplate(id, name, paramsMap) {
        ...
        // Generar inputs basados en el mapa de parámetros <?php echo e(1); ?>, <?php echo e(2); ?>, etc.
    ...
    async function submitTemplate() {
        ...
        const payload = {
            lead_id: <?php echo json_encode($selectedLeadId ?? 0, 15, 512) ?>,
        ...
        try {
            const response = await fetch('/api/whatsapp/templates/send', {
                ...
                headers: {
                    ...
                    'Authorization': 'Bearer ' + <?php echo json_encode(Auth::user()->api_token ?? '', 15, 512) ?>,
