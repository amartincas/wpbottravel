            ...
            <div style="display: flex; flex-direction: column;">
                @foreach($conversations as $conversation)
                    ...
                    <button wire:click="selectConversation('{{ $conversation->customer_phone }}')"
                        style="width: 100%; text-align: left; padding: 1rem; border-bottom: 1px solid #f3f4f6; background: {{ $selectedPhone === $conversation->customer_phone ? '#eff6ff' : 'white' }}; border-left: {{ $selectedPhone === $conversation->customer_phone ? '4px solid #2563eb' : 'none' }}; transition: all 0.2s;">
...
<script>
    ...
    function selectTemplate(id, name, paramsMap) {
        ...
        Object.keys(paramsMap).forEach(key => {
            ...
            div.innerHTML = `
                <label style="font-size: 11px; font-weight: bold; color: #374151;">Dato para {{${key}}} (${paramsMap[key]}):</label>
